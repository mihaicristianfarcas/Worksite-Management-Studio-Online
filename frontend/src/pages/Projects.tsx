...
export default function Projects() {
  ...
  useEffect(() => {
    // Add default sorting to maintain consistent order
    const filtersWithSort = {
      ...filters,
      sortBy: filters.sortBy || 'id', // Default sort by ID
      sortOrder: filters.sortOrder || 'asc' // Default to ascending order
    }
    fetchProjects(filtersWithSort, pagination.page, pagination.pageSize)
  ...
  const resetFilters = () => {
    setTempFilters({})
    setSearchTerm('')
    // Apply default sorting when resetting filters to maintain order
    updateFilters({
      sortBy: 'id',
      sortOrder: 'asc'
    })
  ...
  const refreshCarousel = useCallback(
    (page = pagination.page) => {
      ...
      const filtersWithSort = {
        ...filters,
        sortBy: filters.sortBy || 'id', // Default sort by ID if no sorting specified
        sortOrder: filters.sortOrder || 'asc' // Default to ascending order
      }

      return fetchProjects(filtersWithSort, page, pagination.pageSize).then(() => {
        if (projectIdToKeep) {
          setCurrentProjectId(projectIdToKeep)
        }
      })
    },
    [fetchProjects, pagination.pageSize, currentProjectId, filters]
  )
